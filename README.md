...
[OpenRouter](https://openrouter.ai) puts many model providers behind one
OpenAI-compatible endpoint and one API key. I use it to reach models outside
my main Anthropic and Foundry setup, without a separate account for each
provider. It also publishes usage rankings that show which models people call
the most, and it offers an Auto Router model that picks a model for each
prompt. Read [docs/openrouter.md](docs/openrouter.md) for the details,
including a note on deposit fees.
